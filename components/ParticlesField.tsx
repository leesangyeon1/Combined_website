    ...
    const rebuildGridTargets = () => {
      ...
      for (let r = 0; r < rows; r += 1) {
        for (let c = 0; c < cols; c += 1) {
          gridTargets.push({
            x: Math.floor(offsetX + c * gridGap),
            y: Math.floor(offsetY + r * gridGap),
          });
        }
      }
    ...
    const step = () => {
      ...
      for (const pt of particles) {
        if (isHover) {
          ...
          if (pt.p.x < 0 || pt.p.x > w) {
            pt.v.x *= -1;
            pt.p.x = Math.max(0, Math.min(w, pt.p.x));
          }
          if (pt.p.y < 0 || pt.p.y > h) {
            pt.v.y *= -1;
            pt.p.y = Math.max(0, Math.min(h, pt.p.y));
          }
        }
      }
